...
// build/dev/javascript/gol/ffi.js
var setTimeout = window.setTimeout;

// build/dev/javascript/gol/gol.mjs
var Cell = class extends CustomType {
  constructor(id, is_alive) {
    super();
    this.id = id;
...
var Interval = class extends CustomType {
};
var interval_ms = 200;
function interval_effect() {
  return from(
    (dispatch) => {
      setTimeout(
        () => {
          dispatch(new Interval());
          return void 0;
        },
        interval_ms
      );
...
function update(model, msg) {
  {
    let cells = (() => {
      let _pipe = model.cells;
      return map(
        _pipe,
        (c) => {
          let total_neighbors = (() => {
            let _pipe$1 = get_neighbor_ids(c);
            return fold(
              _pipe$1,
              0,
              (agg, cur) => {
                let neighbor = (() => {
                  let _pipe$2 = model.cells;
                  return find(_pipe$2, (c2) => {
                    return c2.id === cur;
                  });
                })();
                if (neighbor.isOk() && neighbor[0] instanceof Cell && neighbor[0].is_alive) {
                  let alive = neighbor[0].is_alive;
                  return agg + 1;
                } else {
                  return agg;
                }
          ...
          return c.withFields({
            is_alive: (() => {
              ...
              if ($ && (total_neighbors < 2 || total_neighbors > 3)) {
                let n = total_neighbors;
                return false;
              } else if (!$ && total_neighbors === 3) {
                let n = total_neighbors;
                return true;
              } else {
                return c.is_alive;
    ...
    return [new Model2(cells), interval_effect()];
  }
}
function view(model) {
  return div(
    ...
    toList([
      h1(
        toList([class$("text-2xl text-center font-semibold")]),
        toList([text2("Game of life")])
      ),
      div(
        toList([
          style(toList([["max-width", row_width()]])),
...
function init2(_) {
  let cells = (() => {
    let _pipe = range(0, total_cells() - 1);
    return map(
      _pipe,
      (id) => {
        return new Cell(id, random(100) > 50);
      }
    );
  })();
  return [new Model2(cells), interval_effect()];
...
function main() {
  ...
  let $ = start2(app, "#app", new Model2(toList([])));
  if (!$.isOk()) {
    throw makeError(
      "let_assert",
      "gol",
